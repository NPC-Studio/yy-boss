...
///| Script           |  Script Yy File   | Single-Void           |  Gml                    |
///
/// NB: Above, "Single-Void" means that a given Map must have only one key (if multiple are present,
/// the command will abort with an error), and the contents of the key do not matter.
#[derive(Debug, PartialEq, Eq, Clone, Serialize, Deserialize)]
